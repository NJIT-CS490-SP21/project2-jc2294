...
coverage.assign_shortkeys = function () {
  $("*[class*='shortkey_']").each((i, e) => {
    $.each($(e).attr('class').split(' '), (i, c) => {
      if (/^shortkey_/.test(c)) {
        $(document).bind('keydown', c.substr(9), () => {
          $(e).click();
        });
      }
    });
...
coverage.wire_up_help_panel = function () {
  $('#keyboard_icon').click(() => {
    ...
    $('.help_panel').show();
    const koff = $('#keyboard_icon').offset();
    const poff = $('#panel_icon').position();
    $('.help_panel').offset({
...
coverage.wire_up_filter = function () {
  ...
  const table_row_names = table_rows.find('td.name a');
  const no_rows = $('#no_rows');

  // Create a duplicate table footer that we can modify with dynamic summed values.
  const table_footer = $('table.index tfoot tr');
  const table_dynamic_footer = table_footer.clone();
      ...
      if (filter_value === '') {
        ...
        if (hidden > 0) {
          ...
          for (let column = 2; column < 20; column++) {
            ...
            const cells = table_rows.find(`td:nth-child(${column})`);
            if (!cells.length) {
              // No more columns...!
              break;
            }
            ...
            const footer_cell = table_dynamic_footer.find(`td:nth-child(${column})`);
            ...
            if (cells[0].innerHTML.indexOf('%') > -1) {
              ...
              const match = /\.([0-9]+)/.exec(cells[0].innerHTML);
              let places = 0;
              if (match) {
                places = match[1].length;
              }
              const pct = (numer * 100) / denom;
...
coverage.index_ready = function ($) {
  ...
  let sort_list = [];
  const storage_name = 'COVERAGE_INDEX_SORT';
  let stored_list;
  try {
  ...
  const headers = [];
  const col_count = $('table.index > thead > tr > th').length;
...
coverage.pyfile_ready = function ($) {
  ...
  $('.button_toggle_run').click((evt) => {
    coverage.toggle_lines(evt.target, 'run');
  });
  $('.button_toggle_exc').click((evt) => {
    coverage.toggle_lines(evt.target, 'exc');
  });
  $('.button_toggle_mis').click((evt) => {
    coverage.toggle_lines(evt.target, 'mis');
  });
  $('.button_toggle_par').click((evt) => {
  ...
  if (coverage.filters) {
    ...
    coverage.filters = {
      run: false, exc: true, mis: true, par: true,
    };
  }
...
coverage.toggle_lines = function (btn, cls) {
  const onoff = !$(btn).hasClass(`show_${cls}`);
...
coverage.set_line_visibilty = function (cls, onoff) {
  const show = `show_${cls}`;
  const btn = $(`.button_toggle_${cls}`);
  if (onoff) {
...
coverage.chunk_indicator = function (line_elt) {
  const klass = line_elt.attr('class');
  if (klass) {
    const m = klass.match(/\bshow_\w+\b/);
    if (m) {
      return m[0];
    }
  }
...
coverage.line_at_pos = function (pos) {
  const l1 = coverage.line_elt(1);
  const l2 = coverage.line_elt(2);
  let result;
  if (l1.length && l2.length) {
    const l1_top = l1.offset().top;
    const line_height = l2.offset().top - l1_top;
    const nlines = (pos - l1_top) / line_height;
    if (nlines < 1) {
      result = 1;
    } else {
...
coverage.selection_ends_on_screen = function () {
  ...
  const top = coverage.line_elt(coverage.sel_begin);
  const next = coverage.line_elt(coverage.sel_end - 1);
...
coverage.scroll_to_selection = function () {
  ...
  if (coverage.selection_ends_on_screen() < 2) {
    ...
    const top = coverage.line_elt(coverage.sel_begin);
    const top_pos = parseInt(top.offset().top, 10);
...
coverage.build_scroll_markers = function () {
  const c = coverage;
  const min_line_height = 3;
  const max_line_height = 10;
  const visible_window_h = $(window).height();
  ...
  const scroll_marker = $('#scroll_marker');
  const marker_scale = scroll_marker.height() / c.body_h;
  let line_height = scroll_marker.height() / c.lines_len;
  ...
  c.lines_to_mark.each(function () {
    ...
    const line_top = Math.round(offsets[id_name] * marker_scale);
    const line_number = parseInt(id_name.substring(1, id_name.length));
